...
## MapPane — 지도 렌더링 규칙
좌표는 API가 정규화 값(0~1)으로 내려준다(`docs/ADR.md` ADR-014). 정규화 좌표가 이미지 전체를 덮으므로, 역이용안내도를 SVG 좌표계의 `x 0..mapAspect, y 0..1`에 놓으면 지도와 경로가 같은 공간을 공유한다 — 확대도 함께 적용된다. 번들에 없는 지도(`seoul-arex-exploded`)는 배경 없이 경로만 그린다(`docs/ADR.md` ADR-016).
...
3. 정규화 x에 `mapAspect`를 곱해 원래 비율로 되돌린 뒤 그린다. 안 하면 형태가 찌그러진다. `mapAspect`는 번들된 이미지의 실제 크기에서 구하고, 번들에 없을 때만 계약의 `intrinsicWidth/Height`로 떨어진다(`docs/ADR.md` ADR-016).
...
| 요소 | 스펙 |
|------|------|
| 지도 배경 | 등록된 `assetKey`의 이미지를 `x 0..mapAspect, y 0..1`에 `preserveAspectRatio="none"`으로 배치 |
...
### 지도 배경이 없는 항목
- 아직 번들에 없는 지도(현재는 공항철도 층별 도면)를 쓰는 항목은 배경 없이 경로 형태만 그린다. 별도의 "unavailable" 문구는 두지 않는다 — 경로 다이어그램 자체는 진짜 좌표라 유효한 정보다.
